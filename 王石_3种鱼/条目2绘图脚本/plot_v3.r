...
library(ggplot2)
library(reshape2)
df <- read.table('stat2species_common.txt',sep = '\t',header = TRUE)
df <- df[,-1]
df$sample <- c('Carassius auratus','Common','Megalobrama amblycephala')
...
ggsave(filename = 'All_sample_2_2.png',device = 'png',width = 8, height = 6)
ggsave(filename = 'All_sample_2_2.pdf',device = 'pdf',width = 8, height = 6)
