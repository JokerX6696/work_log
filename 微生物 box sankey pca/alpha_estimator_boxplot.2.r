...
for (i in 2:(length(alpha_index)-1)){
	...
	p <- ggplot(data=plotdata, aes(x=Group,y=abundance,fill=Group))+
	  ...
	  geom_boxplot(
	    ...
	    notchwidth = 0.7,
	    outlier.alpha = 0)+
	  # geom_dotplot(binaxis='y', stackdir='centerwhole', dotsize=0.8, shape=16,fill="black")+
	  # geom_jitter(
	  ...
	  #             colour='black',
	  #             position = position_dodge2(jitter.width = 0.4,
	  #                                             jitter.height = 0,
	  #                                             seed=1,
	...
	p <- p+labs(title=title_text, x="",
	            y=substitute(paste(x, " index"), list(x=ylab_text[i])))
	# #添加标题，过长换行
	# pdf(NULL)
	...
	pdf(file=paste0(output, "/alpha_estimator_boxplot", "_", alpha_index[i], ".2.pdf"), width=width, height=height)
	print(p)
	dev.off()
}
# dev.off()
