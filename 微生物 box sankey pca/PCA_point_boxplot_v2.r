...
write.table(sum, "_variance_of_component.txt",quote=F,
            col.names = NA,
            row.names=T,sep="\t")

write.table(pc12_Group, "_pc12_Group0.xls",quote=F,sep="\t")

Data<-pca$x[,1:3]
write.table(Data,'test4',quote=F,row.names=T,sep="\t")
...
# }
# 
pdf( "meta_2bM_PCA_point_boxplot.pdf")
print(p)
dev.off()
