...
library(grid)
library(ggplot2)
toBM <- read.table(file = 'otu_table_even_L7.txt', sep = "\t", header = TRUE, encoding = 'UTF-8')


new_df <- toBM
# 属性矩阵
lab_df <- data.frame(toBM[,1])
names(lab_df) <- 'Specie'
# Cancer组 矩阵
...
samples <- colnames(df)[2:length(colnames(df))]


group <- c(rep('cancer',59),rep('paracancer',30))
mapping <- data.frame(Group=group)
rownames(mapping) <- samples
mapping$Sample <- samples
mapping <- data.frame(Sample=mapping$Sample,Group=mapping$Group)
#--------------------------------------------------------------------------------------------------------------------------------
#args <- commandArgs(T)
...
filedata <- df[,2:90]
rownames(filedata) <- df$Specie
#read.table(args[1],sep="\t",header=T,row.names=1,check.names=F, quote = "")
